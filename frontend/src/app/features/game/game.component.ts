...
export class GameComponent implements OnInit, OnDestroy {
  ...
  onCellClick(cell: CellDto, event: MouseEvent): void {
    ...
    if (this.loading || !this.game || this.game.status !== 'IN_PROGRESS' || cell.state === 'REVEALED' || cell.state === 'FLAGGED') return;
  ...
  onCellRightClick(cell: CellDto, event: MouseEvent): void {
    event.preventDefault();
    if (this.loading || !this.game || this.game.status !== 'IN_PROGRESS' || cell.state === 'REVEALED') return;
  ...
  private checkGameEnd(): void {
    if (this.game && (this.game.status === 'WON' || this.game.status === 'LOST')) {
      ...
      if (this.game.status === 'WON') {
        console.log(`Wygrana w czasie: ${this.secondsElapsed}s`);
      }
    }
  }
}